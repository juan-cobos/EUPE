...
def load_model(size="tiny"):
    ...
    # torch.hub.load clones facebookresearch/EUPE from GitHub on first call,
    # then reuses the cached clone. The weights arg is forwarded to the hub
    # entrypoint, which accepts a local path or URL string.
    model = torch.hub.load(GH_REPO, entrypoint, weights=str(weights_path.resolve()))
...
class EUPEViTEncoder(nn.Module):
    def __init__(self, size="tiny", input_size=256):
        super().__init__()
